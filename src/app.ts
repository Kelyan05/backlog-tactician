...
// Real deploys sit behind a TLS-terminating reverse proxy (Render/Fly/Vercel
...
// Self-hosted deploys (Docker/Render/Fly) ship the built frontend in the same
// image/process as the API, so the browser talks to one origin and the
// session cookie's sameSite=lax just works — no CORS setup needed. Vercel
// deploys the frontend as static output separately instead (see vercel.json),
// so this block is skipped there. In dev, Vite's own server + proxy plays
// the same role.
